...
class MultiProcessCommunication:
		...
		while True:
			try:
				...
				for message in message_list:
					if len(message) <= 0:
						continue
					#elif (message[:3] == 'PROX'): #maybe can remove since we not using proximity
                                        #        self.message_queue.put_nowait(self._format_for(IMAGEREC_HEADER, message[:4] + NEWLINE)) #maybe this line no need anymore
					else:
			...
			try:
				...
				for message in message_list:
					if len(message) <= 0:
						...
						messageArd = message + '|'
						self.message_queue.put_nowait(self._format_for(ARDUINO_HEADER, messageArd.encode()))
	...
	def algorithm_to_android(self, message):
		...
		if len(message_to_send) <= 0:
			...
			self.to_android_message_queue.put_nowait("{m:W1|")
