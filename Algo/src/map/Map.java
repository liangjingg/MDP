	...
	public void resetMap() {
		...
		for (int i = 0; i < Constant.BOARDWIDTH; i++) {
			for (int j = 0; j < Constant.BOARDHEIGHT; j++) {
				...
				if (i >= Constant.BOARDWIDTH - Constant.STARTPOINTWIDTH && j < Constant.STARTPOINTHEIGHT) {
					setGrid(i, j, Constant.POSSIBLEGRIDLABELS[5]); // Set as end point (Top right)
					setDist(i, j, 0);
				}
				// Set the end point grids
				// Set dist to 0 to ensure all values will NOT be overridden
				else if (i < Constant.STARTPOINTWIDTH && j >= Constant.BOARDHEIGHT - Constant.ENDPOINTHEIGHT) {
					setGrid(i, j, Constant.POSSIBLEGRIDLABELS[4]); // Set as startpoint (Bottom left)
					setDist(i, j, 0);
				}
