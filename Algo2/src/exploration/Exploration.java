...
public class Exploration {
    ...
    // private void Limited_Exploration(Robot robot, int time, int percentage, int
    // speed) {
    ...
    // if ((path == null) || (map.getGrid(unexplored[0],
    // unexplored[1]).equals(Constant.UNEXPLORED))) {
    ...
    // corner_calibration(robot);
    // if (!this.image_stop) {
    // this.image_stop = robot.captureImage(new int[][] { { -1, -1, -1 }, { -1, -1,
    // -1 }, { -1, -1, -1 } });
    // }
    ...
    // if ((unexplored) && ((path == null) || (map.getGrid(go_to[0],
    // go_to[1]).equals(Constant.UNEXPLORED)))) {
    ...
    // if ((path == null) || (map.getGrid(go_to[0],
    // go_to[1]).equals(Constant.UNEXPLORED))) {
    // map.setGrid(go_to[0], go_to[1], Constant.OBSTACLE);
    // }
    ...
    private boolean isRightEmpty(Robot robot) {
        ...
        return (!obstacles[3]) && (!obstacles[4])
                && (map.getGrid(pos[0], pos[1]).equals(Constant.EXPLORED)
                        || map.getGrid(pos[0], pos[1]).equals(Constant.STARTPOINT)
    ...
    // }
    // if (valid) {
    // costs[i] = Math.abs(pos[0] - corners[i][0]) + Math.abs(pos[1] -
    // corners[i][1]);
    ...
    // int x = next[0];
    // int y = next[1];
    // int[][] order = new int[][] { { x - 1, y - 2 }, { x, y - 2 }, { x + 1, y - 2
    // }, { x + 2, y - 1 }, { x + 2, y },
    // { x + 2, y + 1 }, { x + 1, y + 2 }, { x, y + 2 }, { x - 1, y + 2 }, { x - 2,
    // y + 1 }, { x - 2, y },
    // { x - 2, y - 1 } };
    // Map map = robot.getMap();

    // for (int[] pos : order) {
    // if ((within_map(pos[0], pos[1])) && (map.getGrid(pos[0],
    // pos[1]).equals(Constant.EXPLORED))) {
    // return pos;
    // }
    // }
    // return null;
    ...
    // private int[] picture_taken(Robot robot, int[] start, int[][]
    // checked_obstacles) {
    // Map map = robot.getMap();
    // int lowest_cost = 9999;
    // int[] cheapest_pos = null;
