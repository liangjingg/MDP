...
public class ConnectionManager extends Thread {
	...
	public String waitingForMessage() {
		...
		while (!complete) {
			...
			// Set the robot position only if the correct message is received and the
			// program is not running exploration and fastest path
			if (!ExplorationThread.getRunning() && !FastestPathThread.getRunning()
					&& s.contains(Constant.INITIALISING)) {
				...
				if ((p_s_s.matcher(s).matches() || p_d_s.matcher(s).matches() || p_s_d.matcher(s).matches()
						|| p_d_d.matcher(s).matches())) {
					complete = true;
					String tmp = s.replace(Constant.INITIALISING + " (", "");
					tmp = tmp.replace(")", "");
					String[] arr = tmp.trim().split(",");
					robot.initialise(Integer.parseInt(arr[1]), Integer.parseInt(arr[0]),
							(Integer.parseInt(arr[2]) + 1) % 4);
			...
			// Start exploration only if the correct message is received and the program is
			// not running exploration and fastest path
			else if (!ExplorationThread.getRunning() && !FastestPathThread.getRunning()
					&& s.equals(Constant.START_EXPLORATION)) {
				s = "Exploration Started";
				thread = ExplorationThread.getInstance(robot, Constant.TIME, Constant.PERCENTAGE, Constant.SPEED,
						Constant.IMAGE_REC);
			...
			// Start fastestpath only if the correct message is received and the program is
			// not running exploration and fastest path
			else if (!ExplorationThread.getRunning() && !FastestPathThread.getRunning()
			...
			// Set waypoint only if the correct message is received and the program is not
			// running exploration and fastest path
			else if (!ExplorationThread.getRunning() && !FastestPathThread.getRunning()
				...
				if ((wp_s_s.matcher(s).matches() || wp_d_s.matcher(s).matches() || wp_s_d.matcher(s).matches()
						|| wp_d_d.matcher(s).matches())) {
					complete = true;
					...
					System.out.printf("Waypoint: x: %d, y: %d \n", robot.getWaypoint().x, robot.getWaypoint().y);
					robot.displayMessage(s, 2);
				}
			}

			// Send mdf arena only if the correct message is received and the program is not
			// running exploration and fastest path
			else if (s.equals(Constant.SEND_ARENA)) {
				String[] arr = robot.getMDFString();
				connectionSocket.sendMessage("{\"map\":[{\"explored\": \"" + arr[0] + "\",\"length\":" + arr[1]
						+ ",\"obstacle\":\"" + arr[2] + "\"}]}");
				robot.displayMessage("{\"map\":[{\"explored\": \"" + arr[0] + "\",\"length\":" + arr[1]
						+ ",\"obstacle\":\"" + arr[2] + "\"}]}", 2);
				complete = true;
			}
